...
N(OP_0x0347),
N(OP_AAAction),
N(OP_AABuy),                    // ← ADD THIS LINE (Laurion's Song - 0x349B)
N(OP_AAExpUpdate),
N(OP_AcceptNewTask),
N(OP_AckPacket),
N(OP_Action),
...
N(OP_CustomTitles),
N(OP_Damage),
N(OP_DragonsHoard),             // ← ADD THIS LINE (Laurion's Song - 0x6D0F)
N(OP_DragonHoardFeatureSetup),  // Laurion's Song - 0x687B - 80 bytes
N(OP_DragonHoardKeepAlive),     // Laurion's Song - 0x706A - 82 bytes (2 + 80), DH heartbeat/keepalive
N(OP_DHUnknown3C92),           // Laurion - 0x3C92 unknown, DH window open State=0
N(OP_DHUnknown6B80),            // Laurion - 0x6B80 unknown, DH window open State=0
N(OP_DHUnknown6D2D),           // Laurion - 0x6D2D unknown size=8, DH window open State=0
N(OP_DHUnknown7F65),           // Laurion - 0x7F65 unknown, DH window open State=0
N(OP_FeatureUnlock),            // Laurion's Song - 0x4451 - Feature unlock packet
N(OP_Death),
...
N(OP_FaceChange),
N(OP_Feedback),
N(OP_FeatureSetup3),          // Laurion's Song - 0x3FE5 - 16 bytes (slot counts for DH/Depot)
N(OP_FeatureSetup4),          // Laurion's Song - 0x229C - 2 bytes (Tradeskill Depot query)
N(OP_FeignDeath),
...
N(OP_ItemName),
N(OP_ItemPacket),
N(OP_ItemPreview),              // ← ADD THIS LINE (Laurion's Song - 0x6159)
N(OP_ItemPreviewRequest),
N(OP_ContainerItemInspect),  // Laurion DH/bag item inspect button 0x609c
...
N(OP_PersonalDepot),           // ← Laurion's Song - 0x41A2 - Personal Tradeskill Depot
N(OP_PersonalDepotFeatureSetup), // Laurion's Song - 0x90B9 - 148 bytes
N(OP_PetBuffWindow),
N(OP_PetCommands),
...
N(OP_WearChange),
N(OP_Weather),
N(OP_WindowResponse),           // ← ADD THIS LINE (Laurion's Song - 0x2B7C)
N(OP_Weblink),
N(OP_WhoAllRequest),
N(OP_WhoAllResponse),
